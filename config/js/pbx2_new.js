...
pbx.voice_menus.parse = function(cxt) {
	...
	try {
		...
		for ( var t in steps ) {
			steps[t].forEach( function(s) {
				return s;
			});		
			// Set the vm.steps variable to the steps derived from the passed in cxt sorted above.
			vm.steps[t] = steps[t];
	
			if(t == 's') {
				/*  Below we grab the name of the Voice Menu from the element 0 from the 's' exten.  We need to make this 
				conditional on 's' when we start processing all the extens. */
				vm.comment = vm.steps[t][0].getNoOp();
			};
		};
		...
		top.log.error(err);
		alert('Error Parsing VoiceMenu. Error info follows.\nErr: ' + err);
	} finally {
...
pbx.voice_menus.add = function(name, menu, callback) {
	...
	del_actions.new_action('delcat', name, '', '');
	del_actions.callActions();*/
	//
	var actions = new listOfActions();
	actions.filename('extensions.conf');
	actions.new_action('delcat', name, '', '');
	alert("invoque delcat for " + name);
	actions.callActions();
	alert("invoque passed " + name);
	...
	if (menu.alias_exten) {
		...
		actions.new_action('delete', ASTGUI.contexts.VoiceMenuExtensions, 'exten', '', menu.alias_exten);
		actions.new_action('append', ASTGUI.contexts.VoiceMenuExtensions, 'exten', menu.alias_exten);
	}

	for(var s in menu.steps){
		for( var t=0; t < menu.steps[s].length ; t++ ){
			actions.new_action('append', name, 'exten', menu.steps[s][t]);
 		}
	}
...
pbx.voice_menus.remove = function(name) {
	...
	var resp = acts.callActions();
	if (!resp.contains('Response: Success')) {
		top.log.error('voice_menus.remove: error updating extensions.conf');
		top.log.error(resp);
		return false;
	}
