	...
	save_VMenu_Advanced : function(){ // VoiceMenus_miscFunctions.save_VMenu_Advanced();
		...
		try{
			...
			vmenu_lines.each( function(this_line){
				...
				console.log("vassilux 1 line : " + this_line);
	...
	push_newstep: function(){ // VoiceMenus_miscFunctions.push_newstep()
		...
		switch(step_action){
			...
			case 'GotoDirecotry':
				...
				break;
			case 'SetMusicOnHold':
				newstep = 'Set(CHANNEL(musicclass)=' + ASTGUI.getFieldValue('newstep_mohClass') + ')';
				//newstep = 'MusicOnHold(' + ASTGUI.getFieldValue('newstep_mohClass') + ')';
				break;
			case 'Hangup':
				newstep = 'Hangup()';
				break;
