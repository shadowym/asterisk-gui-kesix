...
var VoiceMenus_miscFunctions = {
	...
	save_VMenu_Advanced : function(){ // VoiceMenus_miscFunctions.save_VMenu_Advanced();
		...
		//alert("save_VMenu_Advanced");
		//return;

		try{
			//vassilux
			alert(JSON.stringify(EDIT_VMENU));
			/*var del_actions = new listOfActions();
			del_actions.filename('extensions.conf');
			del_actions.new_action('delcat', EDIT_VMENU, '', '');
			del_actions.callActions();*/
			//
			...
			//alert(JSON.stringify(vmenu_lines));
	...
	push_newstep: function(){ // VoiceMenus_miscFunctions.push_newstep()
		...
		if(CURRENT_ACTIONS[_$('vmenu_stepexten').value] == undefined){
			CURRENT_ACTIONS[_$('vmenu_stepexten').value] = [];
		}
		
		CURRENT_ACTIONS[_$('vmenu_stepexten').value].push(_$('vmenu_stepexten').value + ",n," + newstep);
	...
	refresh_allSteps: function(){ // VoiceMenus_miscFunctions.refresh_allSteps()
		ASTGUI.domActions.removeAllChilds( 'sqSteps' );
		var add_sqStep = function(ca,a,i){
			if(!(ca=='s' && a == 0)){
				var txt = CURRENT_ACTIONS[ca][a];
				if(txt==""){ return;}
				var tmp = document.createElement('div');
				tmp.STEPNO = i ;
				/*	TODO: GNUTT Add exten to info	*/
				top.log.debug('CURRENT_ACTIONS[ca][a]: ' + txt);
				var sp_stepexten = document.createElement('span');
					sp_stepexten.classname = 'step_exten';
					sp_stepexten.innerHTML = ASTGUI.parseContextLine.getExten(txt) + ':';
				var sp_desc = document.createElement('span');
					sp_desc.className = 'step_desc';
					sp_desc.innerHTML = ASTGUI.parseContextLine.showAs(ASTGUI.parseContextLine.getAppWithArgs(txt)) ;
		...
		var BOOLEANNoOpExists = false;
		
		for( var t=0; t < CURRENT_ACTIONS['s'].length ; t++ ) {
			if (CURRENT_ACTIONS['s'][t] == 's,1,NoOp(' + _$('vmenu_name').value + ')'){
				BOOLEANNoOpExists = true;
			}
 		}
		
		if (!BOOLEANNoOpExists)	{
			CURRENT_ACTIONS['s'].splice( 0,1,'s,1,NoOp(' + _$('vmenu_name').value + ')' );
		}
		...
		var tmpString = '';
		for( var ca in CURRENT_ACTIONS) {
			//if (CURRENT_ACTIONS.hasOwnProperty(ca)) continue;  It won't display steps if this line is enabled. TODO: GNUTT CHECK
			if( ca != '1' && 
				ca != '2' && 
				ca != '3' && 
				ca != '4' && 
				...
				for( var t=0; t < CURRENT_ACTIONS[ca].length; t++ ){
					if( CURRENT_ACTIONS[ca][t] != undefined){
						add_sqStep(ca,t,i);

						tmpString = ASTGUI.parseContextLine.getAppWithArgs(CURRENT_ACTIONS[ca][t])
						if(t==1 && tmpString.toLowerCase().beginsWith('noop(')){
							CURRENT_ACTIONS[ca][t] = 's,1,NoOp(' + _$('vmenu_name').value + ')';
						}else{
							CURRENT_ACTIONS[ca][t] = ASTGUI.parseContextLine.getExten(CURRENT_ACTIONS[ca][t]) + ',n,' + ASTGUI.parseContextLine.getAppWithArgs(CURRENT_ACTIONS[ca][t]);
						}
						CURRENT_ACTIONS_INDEX.exten.push(ca);
						CURRENT_ACTIONS_INDEX.exten_index.push(t);
						i++;
					}
				}
			}
...
var delete_voiceMenu_confirm = function(a){
	...
	EDIT_VMENU = a ;
	if(!parent.pbx.voice_menus.remove(EDIT_VMENU)){
		alert('Delete Voice Menu failed.');
	}
};
...
var voiceMenu_saveChanges = function(){
	...
	var sortContextByExten = function(cxt , getAll){ // ASTGUI.sortContextByExten( cxt , boolean); 
		...
		for( var THIS_EXTEN in EXTENSIONS ){ if(EXTENSIONS.hasOwnProperty(THIS_EXTEN) ){
			...
			if(THIS_EXTEN != '1' && 
				...
				TMP_CONTEXT.each( function(line, line_index){
					...
					if( priority == 'n' ){
						var new_Priority = Number(ASTGUI.parseContextLine.getPriority(TO_RETURN[THIS_EXTEN][line_index-1])) + 1 ;
	...
	// Reorder the steps so that 'NoOp(' + vm.comment + ')'  comes first and everything else is ordered correctly
	//vm.steps = parent.ASTGUI.sortContextByExten(CURRENT_ACTIONS, true);
	vm.steps = sortContextByExten(CURRENT_ACTIONS, true);
...
var updateVoiceMenus_Table = function(){
	...
	$('#sqSteps').click(function(event){
		...
		var tmpExten = CURRENT_ACTIONS_INDEX.exten[stepNo];
		var tmpIndex = CURRENT_ACTIONS_INDEX.exten_index[stepNo];
		
			switch(cl){
				case 'step_delete':
					CURRENT_ACTIONS[tmpExten].splice(tmpIndex,1);
					CURRENT_ACTIONS_INDEX.exten.splice(stepNo,1);
					CURRENT_ACTIONS_INDEX.exten_index.splice(stepNo,1);
					break;
				case 'step_up':
					if((tmpExten!='s' && stepNo == 0)||(tmpExten=='s' && stepNo == 1)) return;
					var tmp = CURRENT_ACTIONS[tmpExten][tmpIndex] ;
					var tmp2 = CURRENT_ACTIONS_INDEX.exten[stepNo];
					var tmp3 = CURRENT_ACTIONS_INDEX.exten_index[stepNo];
					CURRENT_ACTIONS[tmpExten].splice(tmpIndex, 1);
					CURRENT_ACTIONS[tmpExten].splice(tmpIndex-1, 0, tmp);
					CURRENT_ACTIONS_INDEX.exten.splice(stepNo,1);
					CURRENT_ACTIONS_INDEX.exten.splice(stepNo-1,0,tmp2);
					CURRENT_ACTIONS_INDEX.exten_index.splice(stepNo,1);
					CURRENT_ACTIONS_INDEX.exten_index.splice(stepNo-1,0,tmp3);
					break;
				case 'step_down':
					if(stepNo == (CURRENT_ACTIONS_INDEX.exten.length-1) ) return;
					var tmp = CURRENT_ACTIONS[tmpExten][CURRENT_ACTIONS_INDEX.exten_index[stepNo]] ;
					var tmp2 = CURRENT_ACTIONS_INDEX.exten[stepNo];
					var tmp3 = CURRENT_ACTIONS_INDEX.exten_index[stepNo];
					CURRENT_ACTIONS[tmpExten].splice(tmpIndex+2, 0, tmp);
					CURRENT_ACTIONS[tmpExten].splice(tmpIndex, 1);
					CURRENT_ACTIONS_INDEX.exten.splice(stepNo+2,0,tmp2);
					CURRENT_ACTIONS_INDEX.exten.splice(stepNo,1);
					CURRENT_ACTIONS_INDEX.exten_index.splice(stepNo+2,0,tmp3);
					CURRENT_ACTIONS_INDEX.exten_index.splice(stepNo,1);
					break;
				default:
					break;
