...
var RESET_USER_FORM_FIELDS = function(){ // RESET_USER_FORM_FIELDS();
	ASTGUI.resetTheseFields( ['new_ext','edit_fullname','edit_user_dialplan','edit_OutBoundCallerid','edit_hasvoicemail','edit_vmsecret','edit_email','edit_hasSip','edit_hasIax','edit_fxs','edit_flash','edit_rxflash','codec_one','codec_two','codec_three','codec_fourth','codec_fifth','macaddress','linenumber','edit_secret','edit_nat','edit_canreinvite','edit_dtmfmode','edit_insecure', 'edit_mwifrom', 'edit_3wayCalling','edit_inDirectory','edit_callWaiting','edit_cti','edit_isagent','edit_pickupgroup','edit_linekeys','edit_maxcallnumbers' ,'edit_requirecalltoken', 'edit_record'] );
	...
	if( isNewUSER == false){
		...
		ASTGUI.updateFieldToValue( 'edit_record', uinfo.getProperty('recording') );
		ASTGUI.updateFieldToValue( 'phone_type', uinfo.getProperty('phone_type') );
...
var SAVE_USER_FORM = function(){ // SAVE_USER_FORM();
	...
	if (parent.sessionData.PLATFORM.isABE || parent.sessionData.PLATFORM.isAA50) {
		...
		x.new_action('update', u, 'recording', ASTGUI.getFieldValue('edit_record') );		tmp_obj['recording'] = ASTGUI.getFieldValue('edit_record');
		x.new_action('update', u, 'phone_type', ASTGUI.getFieldValue('phone_type') );		tmp_obj['phone_type'] = ASTGUI.getFieldValue('phone_type');
		var user_record = ASTGUI.getFieldValue('edit_record');
		var res = ASTGUI.ASTDB.updateKey( { dbname: 'KESIX', key : 'USERS/' + u + '/recording', keyvalue : user_record } );
		var phone_type = ASTGUI.getFieldValue('phone_type'); 
		//
		/*if(phone_type !=""){
			res = ASTGUI.ASTDB.updateKey( { dbname: 'KESIX', key : 'USERS/' + u + '/phonetype', keyvalue : phone_type } );
		}*/		
		//alert("user " + u + " record " + user_record + " result " + res);
		var after = function(){
			//generate autoprovioning file
			if( _$('macaddress').value && phone_type !=""){
				parent.ASTGUI.dialog.waitWhile(' The System generate autoprosioning file... ');
				parent.ASTGUI.systemCmd( top.sessionData.directories.script_generateAutoProv + " " + tmp_obj['macaddress'] + " " + phone_type, function(){
